...
setwd('C:\\Users\\62190\\Documents\\카카오톡 받은 파일')
library(readxl)
...
d1 <- read_xlsx('2020 빅콘테스트 데이터분석분야-챔피언리그_2019년 실적데이터.xlsx')
str(d1)
d1 <- as.matrix(d1)
d1%>%head
d1 %>% dim
...
colnames(d1) <- c('day','date','month','time','time(conti)','exposure','brand','codename',
                  'merchandise name','category','price','total price ',
                  'seemean','seemin','seemax','seevar','seemed','seesum',
                  'precipitation','mean temp','bottom temp','top temp')
...
exposure <- as.numeric(d1[,6])

brand <- as.factor(d1[,7])

code_name <- as.factor(d1[,8])

merch_name <- tolower(d1[,9])
...
price <- as.numeric(d1[,11])

total_revenue <- as.numeric(d1[,12])

seemean <- as.numeric(d1[,13])
min(seemean[seemean!=0])
seemean[seemean==0] <- 0.00006

seemin <- as.numeric(d1[,14])

seemax <- as.numeric(d1[,15])
min(seemax[seemax!=0])
seemax[seemax==0] <- 0.001

seevar <- as.numeric(d1[,16])
min(seevar[seevar!=0])
seevar[seevar==0] <- 1.0e-37

seemed <- as.numeric(d1[,17])

seesum <- as.numeric(d1[,18])
min(seesum[seesum!=0])
seesum[seesum==0] <- 0.001

precipitation <- as.numeric(d1[,19])

mean_temp <- as.numeric(d1[,20])

bottom_temp <- as.numeric(d1[,21])

top_temp <- as.numeric(d1[,22])

data0 <- data.frame(day,date,month,time,con_time,exposure,brand,code_name,merch_name,category,price,
                    total_revenue,seemean,seemin,seemax,seevar,seemed,seesum,precipitation,
                    mean_temp,top_temp,bottom_temp)
sum(is.na(data0))
...
xgb1 <- xgb.train(params=def_param1,data=xg_train1,nrounds=59,
